...
class LSTM(object):
  ...
  def __init__(self, inputs_batch, label1_batch, label2_batch, lengths_batch, infer=False):
    ...
    with tf.variable_scope('gpu_variables', reuse=tf.AUTO_REUSE):
      for i_gpu, gpu_id in enumerate(config.GPU_LIST):
        with tf.device('/gpu:%d' % gpu_id):
          with tf.name_scope('tower_%d' % gpu_id):
            with tf.device('/cpu:0'):
              gpu_inputs = inputs_batch
              gpu_mixed = gpu_inputs

              gpu_labels1 = label1_batch
              gpu_labels2 = label2_batch

              gpu_lengths = lengths_batch
              gpu_batch_size = tf.shape(gpu_lengths)[0]
            ...
            with tf.variable_scope('forward1'):
              ...
              outputs = tf.reshape(
                  outputs, [gpu_batch_size, -1, config.rnn_size])
            ...
            if config.model_type.lower() == 'lstm':
              with tf.variable_scope('lstm'):
                cell = tf.contrib.rnn.MultiRNNCell(
                    ...
                    state_is_tuple=True)
                gpu_initial_state = cell.zero_state(
                    self.batch_size, tf.float32)
                state = gpu_initial_state
                outputs, state = tf.nn.dynamic_rnn(
                    cell, outputs,
                    dtype=tf.float32,
                    sequence_length=tf.cast(gpu_lengths, tf.int32),
                    initial_state=gpu_initial_state)
                gpu_final_state = state
            ...
            with tf.variable_scope('forward2'):
              ...
              gpu_activations1 = tf.reshape(
                  mask1, [gpu_batch_size, -1, config.output_size])
              gpu_activations2 = tf.reshape(
                  mask2, [gpu_batch_size, -1, config.output_size])

              gpu_cleaned1 = gpu_activations1*gpu_mixed
              gpu_cleaned2 = gpu_activations2*gpu_mixed

              gpu_loss = utt_PIT_MSE_for_LSTM(gpu_cleaned1,
                                              gpu_cleaned2,
                                              gpu_labels1,
                                              gpu_labels2)
              # tvars = tf.trainable_variables()
            if not infer:
              # grads, _ = tf.clip_by_global_norm(tf.gradients(gpu_loss, tvars),
              #                                   config.max_grad_norm)
              utils.tf_tool.tower_to_collection(tower_batchsize=gpu_batch_size,
                                                tower_losses=gpu_loss,
                                                # tower_grads=grads,
                                                tower_cleaned1=gpu_cleaned1,
                                                tower_cleaned2=gpu_cleaned2)
            else:
              utils.tf_tool.tower_to_collection(tower_batchsize=gpu_batch_size,
                                                tower_cleaned1=gpu_cleaned1,
                                                tower_cleaned2=gpu_cleaned2)

    self._cleaned1 = tf.concat(tf.get_collection('tower_cleaned1'), 0)
    self._cleaned1 = tf.concat(tf.get_collection('tower_cleaned2'), 0)
    self.batch_size = tf.reduce_sum(tf.get_collection('tower_batchsize'))
